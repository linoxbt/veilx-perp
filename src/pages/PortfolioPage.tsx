...
const PortfolioPage = () => {
  ...
  const sections = [
    ...
    { id: "margin", label: "Margin Account", icon: ShieldCheck },
      ...
      <main className="pt-14 flex-1">
        <div className="max-w-[1300px] mx-auto px-3 sm:px-4 py-4 sm:py-6">
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <Link
              to="/trade"
              className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition-colors"
            >
          ...
          {!connected ? (
            ...
            <div className="flex flex-col lg:flex-row gap-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                {activeSection === "margin" && <MarginAccountSection />}
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard label="Margin Balance" value="$0.00" icon={ShieldCheck} />
        <StatCard label="Available Margin" value="$0.00" icon={TrendingUp} />
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard label="Unrealized PnL" value="$0.00" icon={BarChart3} accent="text-profit" />
        <StatCard label="Realized PnL" value="$0.00" icon={DollarSign} />
...
/* ─── Margin Account ─── */
function MarginAccountSection() {
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <StatCard label="Total Collateral" value="$0.00" icon={Wallet} />
        <StatCard label="Used Margin" value="$0.00" icon={ShieldCheck} />
        <StatCard label="Free Margin" value="$0.00" icon={TrendingUp} accent="text-profit" />
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        <StatCard label="Margin Ratio" value="—" icon={Percent} />
        <StatCard label="Maintenance Margin" value="$0.00" icon={Activity} />
        <StatCard label="Liquidation Risk" value="None" icon={Eye} accent="text-profit" />
      </div>

      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="text-sm font-semibold text-foreground mb-3">Margin Health</h3>
        <div className="space-y-3">
          <div>
            ...
            <div className="w-full h-2 rounded-full bg-muted">
              <div className="h-full rounded-full bg-profit" style={{ width: "0%" }} />
            </div>
          </div>
          <div className="text-[10px] text-muted-foreground space-y-1">
            <p>• <strong className="text-foreground">Cross margin</strong> — all collateral backs all positions</p>
            <p>• <strong className="text-foreground">Maintenance margin:</strong> 5% (500 bps) of position value</p>
            <p>• <strong className="text-foreground">Liquidation:</strong> Triggered when margin ratio falls below maintenance</p>
            <p>• <strong className="text-foreground">Privacy:</strong> Margin health computed via Arcium MPC — liquidation levels are never exposed on-chain</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-semibold text-primary">Arcium MPC Protected</span>
        </div>
        <p className="text-[11px] text-muted-foreground">
          Your margin balance, liquidation threshold, and position details are encrypted via Arcium's 3-of-5 MPC. 
          No single node — and no other trader — can see your exposure levels.
        </p>
      </div>
    </div>
  );
...
function FundingSection() {
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="text-sm font-semibold text-foreground mb-3">How Funding Rates Work</h3>
        <div className="text-xs text-muted-foreground space-y-2">
          <p>Funding rates keep perpetual prices aligned with spot prices. They settle every <strong className="text-foreground">1 hour</strong>.</p>
          <p><strong className="text-foreground">Positive rate:</strong> Longs pay shorts — perp price is above spot.</p>
          <p><strong className="text-foreground">Negative rate:</strong> Shorts pay longs — perp price is below spot.</p>
          <p><strong className="text-foreground">Privacy via Arcium:</strong> Aggregate open interest is public (needed for rate calculation), but individual trader exposure is encrypted. No one can see your position size when computing your funding payment.</p>
        </div>
      </div>
    </div>
  );
...
function DepositSection() {
  ...
  const handleFaucetMint = async () => {
    ...
    try {
      ...
      try {
        ...
        transaction.add(createAssociatedTokenAccountInstruction(publicKey, ata, publicKey, USDC_MINT));
      }
      const mintAmount = BigInt(10_000 * 10 ** USDC_DECIMALS);
      transaction.add(createMintToInstruction(USDC_MINT, ata, publicKey, mintAmount));
      transaction.feePayer = publicKey;
      const sig = await sendTransaction(transaction, connection);
      await connection.confirmTransaction(sig, "confirmed");
      toast.success("Minted 10,000 test USDC!", { description: `Tx: ${sig.slice(0, 12)}…` });
      refresh();
    } catch (err: any) {
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const transaction = new Transaction();
      if (asset === "SOL") {
        const [userVaultPda] = PublicKey.findProgramAddressSync([Buffer.from("user_vault"), publicKey.toBuffer()], programId);
        ...
        if (!vaultAtaInfo) {
          transaction.add(createAssociatedTokenAccountInstruction(publicKey, vaultAta, userVaultPda, USDC_MINT));
        }
        const transferAmount = BigInt(Math.round(amtNum * 10 ** USDC_DECIMALS));
        transaction.add(createTransferInstruction(userAta, vaultAta, publicKey, transferAmount));
      }
      transaction.feePayer = publicKey;
      const sig = await sendTransaction(transaction, connection);
      await connection.confirmTransaction(sig, "confirmed");
      toast.success(`Deposited ${amtNum} ${asset}`, { description: `Tx: ${sig.slice(0, 12)}…` });
      setAmount("");
      refresh();
    } catch (err: any) {
      console.error("Deposit failed:", err);
      toast.error(err?.message?.includes("insufficient") ? `Insufficient ${asset} balance` : err?.message || "Deposit failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 sm:p-6 space-y-5 max-w-md">
      <div className="flex items-center gap-3">
        <ArrowDownToLine className="h-6 w-6 text-primary" />
        <div>
          <h2 className="text-lg font-bold text-foreground">Deposit</h2>
          <p className="text-xs text-muted-foreground">Deposit SOL or USDC into your margin account.</p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-border bg-muted/50 p-3">
        ...
        <div className="flex gap-4">
          <div className="flex-1">
            ...
            <p className="text-sm font-mono font-semibold text-foreground">{solBalance !== null ? solBalance.toFixed(4) : "—"}</p>
          </div>
          <div className="flex-1">
            <span className="text-[10px] text-muted-foreground">USDC</span>
            <p className="text-sm font-mono font-semibold text-foreground">{usdcBalance !== null ? usdcBalance.toFixed(2) : "—"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {(["SOL", "USDC"] as const).map((a) => (
          <button key={a} onClick={() => setAsset(a)} className={`flex-1 rounded-lg py-2 text-sm font-semibold transition-colors ${asset === a ? "bg-primary text-primary-foreground" : "border border-border bg-muted text-muted-foreground hover:text-foreground"}`}>
            {a}
          </button>
        ))}
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          {asset === "SOL" && solBalance !== null && (
            <button onClick={() => setAmount(String(Math.max(0, solBalance - 0.01).toFixed(4)))} className="text-[10px] text-primary hover:underline">Max: {solBalance.toFixed(4)}</button>
          )}
          {asset === "USDC" && usdcBalance !== null && (
            <button onClick={() => setAmount(String(usdcBalance.toFixed(2)))} className="text-[10px] text-primary hover:underline">Max: {usdcBalance.toFixed(2)}</button>
          )}
        </div>
        <input type="number" step={asset === "SOL" ? "0.001" : "0.01"} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00"
      ...
      <button onClick={handleDeposit} disabled={loading || !amount} className="w-full rounded-lg bg-primary py-3 text-sm font-bold text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
        {loading ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Confirming…</span> : `Deposit ${asset}`}
      </button>
      ...
      <div className="rounded-lg border border-border bg-muted/30 p-3 space-y-2">
        ...
        <p className="text-[10px] text-muted-foreground">Mint 10,000 VeilX test USDC to your wallet. Your wallet must be the mint authority (see docs).</p>
        <button onClick={handleFaucetMint} disabled={faucetLoading || !connected} className="w-full rounded-lg border border-primary/30 bg-primary/10 py-2 text-xs font-semibold text-primary hover:bg-primary/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          {faucetLoading ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-3 w-3 animate-spin" />Minting…</span> : "Mint 10,000 Test USDC"}
        </button>
      </div>

      <div className="text-[10px] text-muted-foreground">
        {asset === "USDC" && <p>VeilX Test USDC mint: <span className="font-mono">{USDC_MINT.toBase58().slice(0, 16)}…</span></p>}
      </div>
    </div>
  );
}
...
function WithdrawSection() {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const transferAmount = BigInt(Math.round(amtNum * 10 ** USDC_DECIMALS));
      const transaction = new Transaction();
      transaction.add(createTransferInstruction(vaultAta, userAta, userVaultPda, transferAmount));
      transaction.feePayer = publicKey;
      const sig = await sendTransaction(transaction, connection);
      await connection.confirmTransaction(sig, "confirmed");
      toast.success(`Withdrew ${amtNum} USDC`, { description: `Tx: ${sig.slice(0, 12)}…` });
      setAmount("");
    } catch (err: any) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 sm:p-6 space-y-5 max-w-md">
      ...
      <button onClick={handleWithdraw} disabled={loading || !amount} className="w-full rounded-lg bg-primary py-3 text-sm font-bold text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
        {loading ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Confirming…</span> : "Withdraw"}
      </button>
      <p className="text-[10px] text-muted-foreground">Withdrawals require available (unreserved) margin.</p>
    </div>
  );
}
...
function SwapSection() {
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      const [swapVaultPda] = PublicKey.findProgramAddressSync([Buffer.from("swap_vault"), publicKey.toBuffer()], programId);
      ...
      const sig = await sendTransaction(transaction, connection);
      await connection.confirmTransaction(sig, "confirmed");
      toast.success(`Swapped ${amtNum} ${fromAsset} → ${toAsset}`, { description: `Tx: ${sig.slice(0, 12)}…` });
      setAmount("");
      refresh();
    } catch (err: any) {
    ...
    <div className="rounded-xl border border-border bg-card p-5 sm:p-6 space-y-5 max-w-md">
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1">
          ...
          <button onClick={() => setFromAsset(fromAsset === "SOL" ? "USDC" : "SOL")} className="w-full rounded-lg border border-border bg-muted px-3 py-2.5 text-sm font-semibold text-foreground hover:bg-secondary transition-colors">{fromAsset}</button>
        </div>
        <button onClick={() => setFromAsset(fromAsset === "SOL" ? "USDC" : "SOL")} className="mt-4 p-2 rounded-full border border-border hover:bg-secondary transition-colors">
          <Repeat className="h-4 w-4 text-muted-foreground" />
        </button>
        <div className="flex-1">
          <label className="text-[10px] font-medium text-muted-foreground mb-1 block">To</label>
          <div className="w-full rounded-lg border border-border bg-muted/50 px-3 py-2.5 text-sm font-semibold text-foreground">{toAsset}</div>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          {currentBalance !== null && (
            <button onClick={() => setAmount(String(fromAsset === "SOL" ? Math.max(0, currentBalance - 0.01).toFixed(4) : currentBalance.toFixed(2)))} className="text-[10px] text-primary hover:underline">
              Max: {fromAsset === "SOL" ? currentBalance.toFixed(4) : currentBalance.toFixed(2)}
            </button>
          )}
        </div>
        <input type="number" step={fromAsset === "SOL" ? "0.001" : "0.01"} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00"
          className="w-full rounded-lg border border-border bg-muted px-3 py-2.5 text-sm font-mono text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
      </div>

      <button onClick={handleSwap} disabled={loading || !amount} className="w-full rounded-lg bg-primary py-3 text-sm font-bold text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
        {loading ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" />Swapping…</span> : `Swap ${fromAsset} → ${toAsset}`}
      </button>
